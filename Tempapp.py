...
with st.sidebar.expander("Input Parameters (SI units)", expanded=True):
    ...
    v = st.number_input("Sliding velocity v [m/s]", 0.25, 1.0)
    r = st.number_input("Sliding radius r [m]", 0.009, 0.019)
    r_d = st.number_input("Steel disc radius r_d [m]", 0.025, 0.060)
    b = st.number_input("Steel disc thickness b [m]", 0.005, 0.015)
    b_PTFE = st.number_input("PTFE thickness b_PTFE [m]", 0.005, 0.015)
